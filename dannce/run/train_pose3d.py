...
from dannce.engine.models.pose3d.voxelpose import VoxelPose
from dannce.engine.models.pose3d.learnable_triangulation import VolumetricTriangulationNet
...
def generate_Gaussian_target(all_labels, heatmap_size=[64, 64], ds_fac=4, sigma=2):
    """
    Generate 2D Gaussian targets surrounded at some ground truth position.
    """
    all_targets = []
...
def load_data_into_mem(params, logger, partition, n_cams, generator, train=True):
    ...
    # the heatmaps are 4x downsampled compared to the (resized) input images
    custom_params = params["custom_model"]
    heatmap_size = [custom_params["resize_size"]//4, custom_params["resize_size"]//4]

    # initialize data structures hosted in memory
    X = torch.empty(
        (n_samples, n_cams, params["chan_num"], custom_params["resize_size"], custom_params["resize_size"]), 
        dtype=torch.float32
    )
    ...
    for i in tqdm(range(n_samples)):
        ...
        X[i] = torch.stack(rr[0][0][0], dim=0) #[n_cams, 3, H, W]
...
def predict(params):
    ...
    for idx, i in enumerate(pbar):
        ...
        images = torch.stack(rr[0][0][0], dim=0) #[BS, 6, 3, 256, 256]
